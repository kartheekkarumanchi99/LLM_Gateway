...
// Factory for any OpenAI-compatible provider (same /chat/completions wire format).
// Parameterized by slug + base URL, so one implementation serves openai, deepseek,
// x-ai, mistral, google (OpenAI-compat endpoint), moonshot, perplexity, and more.
export function createOpenAICompatibleAdapter(slug: string, baseUrl: string): ProviderAdapter {
  return {
    slug,
    ...
    async chatStream(upstreamModel, body, apiKey) {
      const res = await fetch(`${baseUrl}/chat/completions`, {
        ...
        body: JSON.stringify({
          ...
          model: upstreamModel,
          stream: true,
          // Ask the provider to include a final usage chunk so we can bill accurately.
...
export const openaiAdapter = createOpenAICompatibleAdapter('openai', config.openaiBaseUrl);
